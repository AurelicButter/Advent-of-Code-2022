...
class Monkey {
	...
	setPointers(trueInstruct: string, falseInstruct: string) {
	...
	inspectItem() {
		...
		currItem = currItem % commonDivisor;
...
for (let i = 0; i < values.length; i += 7) {
	...
	newMonkey.setPointers(values[i + 4], values[i + 5]);

	monkeys.push(newMonkey);
}

const commonDivisor = monkeys.map((n) => n.testNum).reduce((a, b) => a * b, 1);
